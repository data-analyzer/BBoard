...
--
-- 삭제하고 명칭 변경 적용하여 새로 만듦 (적용 인덱스 post-v2)
...
-- 현재
ngram_4_filter 를 ac_index_analyzer, fc_index_analyzer 두 곳에서 사용중
필드는 title_ac, title_fc
     content_ac, content_fc

흐름표 는 제안되는데 ㅎㄿ 제안 안됨
국세청 은 제안되는데 ㄳㅊ 제안 안됨
fc_index_analyzer 의 ngram4_filter => ngram3_filter 로 변경 (단, title은 괜찮을텐데, content는 데이터양이 많을 것임 -> content는 edge_ngram 도 괜찮을듯 한데..)
content_fc 필드에 적용할 fc_content_index_analyzer 와 fc_content_search_analyzer 를 만들어 분리하는 것도 차후 고려

일단, ngram3_filter로 변경
DELETE _index_template/post-template
PUT _index_template/post-template
{
  "index_patterns": ["post-v*"],
...
GET post-v3/_search?_source_excludes=data,attachment,content

///////// edge_ngram 예제 시작 /////////
PUT my-autocomplete-index-000001
{
  "settings": {
    "analysis": {
      "filter": {
...
PUT my-autocomplete-index-000001/_doc/1
{
  "text": "Quick Brown Fox"
}
PUT my-autocomplete-index-000001/_doc/2
{
  "text": "Quick Brother"
}

GET my-autocomplete-index-000001/_search
{
  "query": {
    "match": {
      "text": {
